...
No supported process-local system-home source override exists in the inspected HOST path. `getSystemCodexHomePath()` in `src/main/codex/codex-home-paths.ts` unconditionally returns `join(homedir(), '.codex')`; `ORCA_USER_DATA_PATH` changes only the managed target, while `CODEX_HOME`/`ORCA_CODEX_HOME` are used for real-home lane routing and do not change the source passed to `syncSystemCodexResourcesIntoManagedHome()`. This is not only a resource-junction concern: `runtime-home-service.ts` also uses `getSystemCodexHomePath()/auth.json` in `writeSystemDefaultAuth`, including refreshed-credential writeback, so an isolated profile could write daily credentials as well.

The precise minimum pending decision is a narrow source change: add an explicit experiment system-home source override for `getSystemCodexHomePath()` that is used only by the candidate process and defaults to the current value. Resource synchronization and authentication writeback must both remain inside that experiment directory, and authentication must use only an independent copy or experimental login already covered by existing authorization. This is outside the current no-source-expansion scope and has not been implemented or executed; it does not claim that no authentication route exists.
...
Decide whether to authorize the narrow, candidate-process-only `getSystemCodexHomePath()` experiment-source override described above. Only after that unimplemented change confines both resource synchronization and `writeSystemDefaultAuth` to the experiment directory, and an already-authorized independent authentication copy or experimental login is available, may the bounded Run → Task → `worker-start` / observable `worker-stop` path be reconsidered with the same one-task, one-stop, one-attempt limits.
